...
"""
from utils import prepare_sub_folder, write_loss, get_config, write_translation_images, write_lane_images, Timer
import argparse
...
from utils.common import viz_lanes
...
import warnings
warnings.filterwarnings("ignore", category=UserWarning) # Temp fix for UserWarning
...
for epoch in range(start_epoch, config['max_epoch']):
    ...
    for image_label_a, image_b in tqdm(zip(train_loader_a, train_loader_b), total=iter_per_epoch):
        images_a, det_label, cls_label, seg_label, _ = image_label_a
    ...
    # Write train metrics
    log_dict = trainer.get_metric_log_dict()
    ...
    if (epoch + 1) % config['image_save_epoch'] == 0:
        with torch.no_grad():
            train_preds = trainer.eval_lanes_a(train_display_images_a)
            test_preds = trainer.eval_lanes(test_display_images_b)
        ...
        test_viz = viz_lanes(
            det_out=test_preds[0],
            seg_out=test_preds[2],
            dataset_root=config['dataB_root'],
            names=test_names_b,
            model_in_dims=(config['input_width'], config['input_height']),
            griding_num=config['lane']['griding_num'],
            row_anchor=val_loader_b.dataset.row_anchor,
            num_lanes=config['lane']['num_lanes']
        )

        write_lane_images(train_viz, display_size, epoch + 1, 'train', step=iterations)
        write_lane_images(test_viz, display_size, epoch + 1, 'test', step=iterations)

    # Write translation images
    if not no_adv_gen and (epoch + 1) % config['image_save_epoch'] == 0:
        ...
        write_translation_images(test_image_outputs, display_size, epoch + 1, 'test', step=iterations)
        write_translation_images(train_image_outputs, display_size, epoch + 1, 'train', step=iterations)
