"""
Credit to https://github.com/HsinYingLee/DRIT/blob/master/src/networks.py
"""
...
#------------------------- Basic Functions -------------------------
####################################################################
def get_scheduler(optimizer, hyperparameters, cur_ep=-1):
  if hyperparameters["lr_policy"] == 'lambda':
    def lambda_rule(ep):
      lr_l = 1.0 - max(0, ep - hyperparameters["n_ep_decay"]) / float(hyperparameters["n_ep"] - hyperparameters["n_ep_decay"] + 1)
      return lr_l
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule, last_epoch=cur_ep)
  elif hyperparameters["lr_policy"] == 'step':
    scheduler = lr_scheduler.StepLR(optimizer, step_size=hyperparameters["n_ep_decay"], gamma=0.1, last_epoch=cur_ep)
  else:
